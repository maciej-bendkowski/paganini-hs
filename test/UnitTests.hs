...
assertApprox msg target val eps =
  unless
    ...
    (assertFailure msg')
 where
  msg' = msg ++ ": expected " ++ show target ++ " was " ++ show val
...
testOutput msg eps out = testPaganini testCase'
 where
  testCase' = assertEqualList msg eps out
...
testSimpleTrees =
  testCase "Simply-generated trees" $
...
testCyclicDecompositions =
  testCase "Cyclic decompositions" $
